...
class RouterCopy
{
    ...
    public static function run(): void
    {
        ...
        if (class_exists($controllerClass) && method_exists($controllerClass, $action)) {
            ...
            // If the action expects an integer parameter, cast it to int
            if($numberOfRequiredParams > 0 && $method->getParameters()[0]->hasType()) {
                $firstParameterType = $method->getParameters()[0]->getType()->getName();
                if($firstParameterType === 'int' && isset($params[0])) {
